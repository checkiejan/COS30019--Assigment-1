...
def createButtons(lst):
        ...
        lst.append(cus2)
        x =(screen_width - (width +padding)*3)/2 +40
        y += height + padding
        dfsr = Button(x,y,width +100,height,"DFS recursive")
        dfsr.setSearch(DFSRecursivesearch)
        lst.append(dfsr)
...
if len(sys.argv) == 3:
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                for button in buttons:
                    if button.onPoint(pygame.mouse.get_pos()):
                        grid.resetGrid()
                        search = True
                        strategy = button.search() #get the search function
        ...
        if search and strategy is not None:
            ...
            print(f'{sys.argv[1]} {name} {node}') #print the method
            print(encodePath(path)) #print the path
            search = False
            drawPath = True
